...
import { ShowbarContext } from './App'
import TaskbarProcess from './TaskbarProcess'
...
const getTime = () => {
    ...
    const section = hours >= 12 ? "PM" : "AM"
    const time = (hours % 12).toString() + ":" + minutes.toString() + " " + section
    return time
}

function Taskbar({processes, processDispatch}) {

    const dispatch = useContext(ShowbarContext)
    ...
    const renderProcesses = () => {
        let list = processes.map((process) => {
            return (
                <TaskbarProcess key={process.id} process={process} processDispatch={processDispatch}></TaskbarProcess>
            )
        })
        return list
    }
